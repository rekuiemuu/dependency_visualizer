...
def get_commits_with_file_hash(repo_path, file_hash):
    ...
    for commit in repo.iter_commits():
        ...
        if not parents:
            continue  # Пропускаем коммиты без родителей
        try:
    ...
    # Добавляем родительские коммиты
...
def visualize_graph(commits, repo_path):
    ...
    # Сохраняем граф в файл и отображаем
    with open("graph.dot", "w") as f:
        f.write(graph)
    
    # Используем Graphviz для рендеринга графа
    source = Source(graph)
    source.render("graph", format="png", cleanup=True)  # Сохраняет граф в файл graph.png
